...
function formActionSeletor(action,method,ele){//form 설정함수
  ele.action = action
  ele.method = method
}

function tagMaker(tag,parent,id,callback){ //createElement+appendChild 생성기
...
function multiTagMaker(tag,parent,num){//createElement+appendChild 반복생성기
  for(let i =0;i<num;i++){
    const element = document.createElement(tag)
    parent.appendChild(element)
  }
...
function felexEditor(display,flexDirection,alignItems,justifyContent,ele){//flex 설정함수
...
function layoutEditor(wid,hei,background,ele){//layout 설정함수
  ele.style.width = wid
  ele.style.height = hei
  ele.style.background=background
}

function positionEditor(position,top,bottom,left,right,ele){//position 설정함수
...
function contentFromMaker(parent){     //@ 컨텐츠 부분 자동 생성
  for(let i=0;i<tempData.length;i++){ //tempData = 임시객체
...
//서치바-검색폼-셀렉트+옵션
const searchOptionSelect = tagMaker("select",searchBarForm,"searchOptionSelect")
...
//서치바-글쓰기폼-글쓰기버튼
const createForm = tagMaker("from",searchWrap,"createForm")
