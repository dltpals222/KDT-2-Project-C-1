...
dataPromises.then(([data1, data2]) => {
  ...
  const headerSearchWrap = multiAndSingleTagMaker(headerSearch, "div", "header-wrap");
  ...
  const headerSearchSelect = multiAndSingleTagMaker(headerSearchWrap, "select", { name: "hsSelect", id: "hs-select" });
  ...
  kingGodFlexEditor(headerSearch, "column", "center", "");
  ...
  headerSearchSelect.addEventListener("change", function () {
    ...
    if (selectedOption === "ingredients") {
      ...
      headerSearchInput.addEventListener("input", () => {
        ...
        const matchedResults = data1.ingredients.filter((item) => item.toLowerCase().includes(searchText)).slice(); // 최대 10개의 검색 결과 표시
      ...
      function renderAutocompleteResults(results) {
        ...
        results.forEach((result) => {
          ...
          allMightyStyleEditor(listItem, mainSearchTextItemCss, function (element) {
      ...
      headerSearchInput.addEventListener("input", () => {
        ...
        const matchedResults = data2Results.filter((item) => item.toLowerCase().includes(searchText)).slice();
      ...
      function renderAutocompleteResults(results) {
        ...
        results.forEach((result) => {
          ...
          allMightyStyleEditor(listItem, headerSearchTextItemCss, function (element) {
            element.textContent = result;
          });
          headerSearchText.appendChild(listItem);
